...
def create_chat_interface(conversation):
    ...
    filtered_conversation = [turn for turn in conversation if turn.get('role') != 'system']
    
    messages = []
    
    for i, turn in enumerate(filtered_conversation):
        ...
        if role == 'tool':
            ...
            content_div = create_tool_cards(content or "")
        elif role == 'assistant':
            # For assistant messages, only show content within model_thinking tags
            if content:
                # Remove <thinking>...</thinking> tags but keep <model_thinking>...</model_thinking>
                cleaned_content = re.sub(r'<thinking>.*?</thinking>', '', content, flags=re.DOTALL)
                content_div = html.Div(cleaned_content.strip(), style=styles['message_content'])
            else:
                # Handle null content
                content_div = html.Div("", style=styles['message_content'])
        else:
            # Default for user messages
            content_div = html.Div(content or "", style=styles['message_content'])
...
def update_output(contents, filename):
    ...
    try:
        if 'json' in filename:
            ...
            monitoring_response = data.get('monitoring_response', 'No monitoring response available')
            # Calculate trajectory length from conversation (excluding system messages)
            trajectory_length = len([turn for turn in conversation if turn.get('role') != 'system'])
            
            return html.Div([
                # Verdict Card with Reveal Button
                html.Div(style=styles['verdict_card'], children=[
                    html.H3("🔍 Analysis Verdict", style={'margin': '0 0 15px 0', 'fontSize': '1.5rem'}),
                    html.Div([
                        html.Button("Reveal Analysis", id="reveal-button", style={
                        ...
                        html.Div(id="monitoring-response", style={'display': 'none', 'marginTop': '20px'})
                    ])
                ]),
                
                # Hidden div to store monitoring response data
                html.Div(id="monitoring-data", style={'display': 'none'}, children=monitoring_response),
                ...
                html.Div(style=styles['metrics_grid'], children=[
                    ...
                    html.Div(style=styles['metric_card'], children=[
                        html.Div(sum(len((t.get('content') or '').split()) for t in conversation), style=styles['metric_value']),
...
# --- Callback for Reveal Button ---
@app.callback(
    Output('monitoring-response', 'children'),
    Output('monitoring-response', 'style'),
    [Input('reveal-button', 'n_clicks')],
    [State('monitoring-data', 'children')]
)
def reveal_monitoring_response(n_clicks, monitoring_data):
    if n_clicks and n_clicks > 0:
        return (
            html.Div([
                html.H4("📊 Monitoring Analysis", style={'margin': '0 0 15px 0', 'color': modern_theme['text']}),
                html.Pre(monitoring_data, style={
                    'backgroundColor': modern_theme['background'],
                    'color': modern_theme['text'],
                    'padding': '16px',
                    'borderRadius': '8px',
